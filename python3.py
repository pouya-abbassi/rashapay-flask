...
import urllib.parse, urllib.request			# Python3 only
...
def req():
	...
	name = urllib.parse.quote(request.form['name'].encode('utf-8'))		# Buyer name | You can also inser UTF-8 text
	mobile = request.form['mobile']						# Buyer phone number
	description = urllib.parse.quote(request.form['description'].encode('utf-8'))	# Max 200 character | You can also insert UTF-8 text

	data = urllib.parse.urlencode({"consumer_key":consumer_key,"amount":amount,"email":email,"name":name,"orderid":orderid,"callback":callback,"mobile":mobile,"description":description})	# Generating data for creating the request.
	data = data.encode('UTF-8')		# Python3 only
	u = urllib.request.urlopen("http://rashapay.com/srv/rest/rpaypaymentrequest", data)	# Sending data to the webservice url.

	obj = u.read()	# Reading received data
	obj = obj.decode('UTF-8')	# Python3 only
